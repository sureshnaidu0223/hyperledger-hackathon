  ...
  //  args.push(userName);
    ...
    getUser(userName, function (err, user) {
        if (err) {
            ...
            tx.on('complete', function(results) {
                logger.info("[SDK] completed query123: %j",results.result.toString());
		//remove ,
		var resultWithComma = results.result.toString();
		var resultWithoutComma = "";
		if(resultWithComma.length >= 4){
		var lastComma =  resultWithComma.lastIndexOf(",")
		logger.info("lastComma index %s",lastComma)
			resultWithoutComma = resultWithComma.substr(0, resultWithComma.length-2)  +"]";

		} 
		logger.info("[SDK] completed without comma: %j",resultWithoutComma);	
                cb(null, JSON.parse(resultWithoutComma))
            });
            tx.on('error', function(err) {
